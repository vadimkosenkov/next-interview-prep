...
export default function QuizClient({ blockId, topicId, questions, }: QuizClientProps) {
  ...
  if (result) {
    ...
    const label =
      pct === 100
        ? translations.quiz.resultPerfect
        : pct >= 80
          ? translations.quiz.resultGreat
          : pct >= 60
            ? translations.quiz.resultOk
            : translations.quiz.resultMore;
